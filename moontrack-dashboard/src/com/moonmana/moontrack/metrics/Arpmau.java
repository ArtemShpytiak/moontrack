...
import com.moonmana.moontrack.metrics.charts.ArpmauChart;

import hive.model.filter.FilterHub;

public class Arpmau extends MetricWithFilters {

	public Arpmau(FilterHub filter) {
		super(filter);
		setType(MetricType.ARPMAU);
	}
	...
	public String getSQL() {
		...
		// @formatter:off
				...
				" FROM " + getFromTablesOuter() +
				 " activities" +
				    	" INNER JOIN ("
					    	+ " SELECT date_trunc('month', iaps.date) AS month, SUM(price) AS sumprice"
				    ...
				    + " GROUP BY month"
				    + " ) AS iaps ON month = activities.date"
				+ " WHERE TRUE" +
					getWhereClause() +
					getGameWhereClause() +
					getRegistrationWhereClause() +
